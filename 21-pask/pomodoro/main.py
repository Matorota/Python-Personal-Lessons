...
def reset_timer():
    global cycles
    cycles = 0
    window.after_cancel(timer) # nuima taimeri tampa none
    status.config(text = "")
...
def count_time(count):
    count_min = math.floor(count / 60) # math floor nuima kalblely
    ...
    if count > 0:
        ...
        start_timer()
        marks = ""
        time_completed = math.floor(cycles / 2)
        for _ in range(time_completed):
            marks +="✓"
